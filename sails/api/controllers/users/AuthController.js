...
module.exports = {
  registerUser: async function(req, res) {
    ...
    try {
      ...
      sails.log.debug('REGISTER_ERR::: ' + err);
  ...
  loginUser: async function(req, res){
    ...
    if(!usr){ usr = await Member.findOne({email: req.param('username')}); }
  ...
  refreshUserToken: async function(req, res){
    ...
    var tkn = await RefreshToken.findOne({refreshToken: req.param('token')});
    if(!tkn){ return res.status(403).send(); }
    jwt.verify(req.param('token'), sails.config.jwts.REFRESH_TOKEN_SECRET, (err, user) => {
      if(err){ return res.status(403).send(); }
      delete user['iat']; // Remove autogenerated attributes
