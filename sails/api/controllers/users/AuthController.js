...
module.exports = {
  ...
  loginUser: async function(req, res){
    ...
    try{
      if(await bcrypt.compare(req.param('password'), usr.password)){
        const tokenData = {name: usr.userName, id: usr.id};
  ...
  logoutUser: async function(req, res){
    if(req.param('token')){
      var usr = await sails.helpers.authenticateRefreshToken(req.param('token'));
    ...
    if(req.headers['authorization'] && req.headers['authorization'].split(' ')[1]){
      var usr = await sails.helpers.authenticateJwt(req.headers)
      if(!usr){ return res.status(403).send(); };
      await RefreshToken.destroy({uid: usr.id});
      return res.ok();
    }
